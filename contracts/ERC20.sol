...
contract ERC20 is ERC20PausableUpgradeable, OwnableUpgradeable {
    ...
    uint256 private constant _maxTotalSupply =
        600_000_000 * (uint256(10)**uint256(18));
    ...
    function _beforeTokenTransfer(
        ...
        if (_msgSender() != owner()) {
            ...
            require(
                !isPausedDisabled(),
                "ERC20: token transfer/mint/burn while paused by block number"
            );
        }
    ...
    function mint(uint256 _amount) public onlyOwner {
        require(
            getMaxTotalSupply() >= totalSupply().add(_amount),
            "ERC20: Can't Mint, it exceeds the maximum supply "
        );
        _mint(owner(), _amount);
    }
